...
st.write("")
team_data = [
    ("assets/team/team_diar.jpg",     "DIAR ALYA\nADRIANA",         "ENGINEER"),
    ("assets/team/team_zhaafira.jpg", "ZHAAFIRA AFIFAH\nPUTRI",     "PENULIS LAPORAN"),
    ("assets/team/team_nadin.jpg",    "NADIN CAHYA\nFEBRIANTI",     "PENULIS LAPORAN"),
    ("assets/team/team_fardhan.jpg",  "FARDHAN NAFIISAH\nPUTRA",    "RESEARCH"),
]
cols = st.columns(4)
for c, (photo, name, role) in zip(cols, team_data):
    with c:
        ...
        st.markdown(
            f'<div class="pf-team-name">{name_html}</div>'
            f'<div class="pf-team-role">{role}</div></div>',
            unsafe_allow_html=True,
        )
