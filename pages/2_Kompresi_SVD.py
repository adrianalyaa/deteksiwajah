...
section_header(
    "✦ COBA SEKARANG",
    "🖼️ KOMPRESI WAJAH",
    "Upload foto wajah. Sistem mengubahnya ke grayscale lalu mengompresi menggunakan "
    "SVD. Atur nilai k untuk mengontrol kualitas vs ukuran.",
)
...
with col_ctrl:
    ...
    with stats_box:
        st.markdown('<div class="pf-card-dark">', unsafe_allow_html=True)
        st.markdown('<span class="pf-label-light" style="font-size:7px;">STATISTIK KOMPRESI</span>', unsafe_allow_html=True)
        stats = st.session_state.svd_stats
        if stats:
            st.markdown(
                f'<div style="font-family:\'VT323\',monospace;font-size:17px;color:var(--pink-light);line-height:1.8;">'
                f"Dimensi: {stats['width']}×{stats['height']}<br/>"
                f"Nilai disimpan: {stats['comp_vals']:,}<br/>"
                f"Nilai asli: {stats['orig_vals']:,}<br/>"
                f"Efisiensi: {stats['saved_pct']:.1f}% lebih kecil</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<div style="font-family:\'VT323\',monospace;font-size:17px;color:var(--pink-light);">'
                "— Belum ada foto —</div>",
                unsafe_allow_html=True,
            )
...
with col_out:
    st.markdown('<span class="pf-label">HASIL KOMPRESI (GRAYSCALE)</span>', unsafe_allow_html=True)
    if st.session_state.svd_result is not None:
        ...
        st.markdown(
            f"**k={result_stats['k']}** · Ruang disimpan **{result_stats['saved_pct']:.1f}%**"
        )
...
if run_svd:
    if not file_c:
        ...
        with st.spinner(f"⏳ Memproses SVD k={k_value}... (harap tunggu beberapa detik)"):
